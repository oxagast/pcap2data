...
} = require("./main-frontend/themes-catalog");
const {
  createPacketDetailViewHelpers,
} = require("./main-frontend/packet-detail-view");
const {
  createReleaseNotesHelpers,
} = require("./main-frontend/release-notes");
const {
  createSettingsDiagnosticsHelpers,
} = require("./main-frontend/settings-diagnostics");
const {
  createPluginManagementHelpers,
} = require("./main-frontend/plugin-management");
...
// Theme/ollama-model state used to live here as module-level `let`s.
// They are now owned by the `themesCatalogState` object passed to
// ``createThemesCatalogHelpers`` below (see the factory invocation
// further down). The factory reuses the same array/Set references
// so existing call sites that read/write them through the returned
// helpers observe the same identity.
const themesCatalogState = {
  availableThemes: [],
  availableOllamaModels: [],
  defaultThemeLogoSrc: null,
...
// Live-state bridge for the settings-diagnostics factory. The
// factory reads/writes the cached probe payloads, the in-flight
// promises, the dedupe timestamps, and ``ollamaVersionCheckPassed``
// through this object so the orchestrator's module-level ``let``s
// stay authoritative and the factory stays a pure module webpack
// can tree-shake.
const settingsDiagnosticsState = {
  get ollamaVersionCheckPassed() { return ollamaVersionCheckPassed; },
  set ollamaVersionCheckPassed(v) { ollamaVersionCheckPassed = v; },
  get cachedLlmDiagnostics() { return cachedLlmDiagnostics; },
  set cachedLlmDiagnostics(v) { cachedLlmDiagnostics = v; },
  get cachedBackendDiagnostics() { return cachedBackendDiagnostics; },
  set cachedBackendDiagnostics(v) { cachedBackendDiagnostics = v; },
  get cachedVirusTotalDiagnostics() { return cachedVirusTotalDiagnostics; },
  set cachedVirusTotalDiagnostics(v) { cachedVirusTotalDiagnostics = v; },
  get virusTotalDiagnosticsInFlight() { return virusTotalDiagnosticsInFlight; },
  set virusTotalDiagnosticsInFlight(v) { virusTotalDiagnosticsInFlight = v; },
  get virusTotalDiagnosticsLastSuccessAt() { return virusTotalDiagnosticsLastSuccessAt; },
  set virusTotalDiagnosticsLastSuccessAt(v) { virusTotalDiagnosticsLastSuccessAt = v; },
  get cachedHashesComDiagnostics() { return cachedHashesComDiagnostics; },
  set cachedHashesComDiagnostics(v) { cachedHashesComDiagnostics = v; },
  get hashesComDiagnosticsInFlight() { return hashesComDiagnosticsInFlight; },
  set hashesComDiagnosticsInFlight(v) { hashesComDiagnosticsInFlight = v; },
  get hashesComDiagnosticsLastSuccessAt() { return hashesComDiagnosticsLastSuccessAt; },
  set hashesComDiagnosticsLastSuccessAt(v) { hashesComDiagnosticsLastSuccessAt = v; },
  get cachedHashesComLastLookup() { return cachedHashesComLastLookup; },
  set cachedHashesComLastLookup(v) { cachedHashesComLastLookup = v; },
  get cachedMetricsDiagnostics() { return cachedMetricsDiagnostics; },
  set cachedMetricsDiagnostics(v) { cachedMetricsDiagnostics = v; },
  get metricsDiagnosticsInFlight() { return metricsDiagnosticsInFlight; },
  set metricsDiagnosticsInFlight(v) { metricsDiagnosticsInFlight = v; },
  get metricsDiagnosticsLastSuccessAt() { return metricsDiagnosticsLastSuccessAt; },
  set metricsDiagnosticsLastSuccessAt(v) { metricsDiagnosticsLastSuccessAt = v; },
};
...
// Live-state bridge for the plugin-management factory. The factory
// reads/writes the cached plugin registry, the error log, the
// loaded-plugin id set, the selected row id, and the in-flight
// capability-dialog resolver through this object so the orchestrator's
// module-level bindings stay authoritative and the factory stays a
// pure module webpack can tree-shake.
const pluginManagementState = {
  get cachedPluginRegistry() { return cachedPluginRegistry; },
  set cachedPluginRegistry(v) { cachedPluginRegistry = v; },
  get pluginErrorEntries() { return pluginErrorEntries; },
  set pluginErrorEntries(v) { pluginErrorEntries = v; },
  get loadedPluginIds() { return loadedPluginIds; },
  // ``loadedPluginIds`` is a const ``Set`` so no setter is needed.
  get selectedPluginId() { return selectedPluginId; },
  set selectedPluginId(v) { selectedPluginId = v; },
  get activePluginInstallCapabilityDialogResolver() { return activePluginInstallCapabilityDialogResolver; },
  set activePluginInstallCapabilityDialogResolver(v) { activePluginInstallCapabilityDialogResolver = v; },
};
...
// Live-state bridge for the release-notes factory. The factory
// accesses the release-cluster state through this object so its
// reads/writes stay in lockstep with the orchestrator's
...
// ============================================================================
// Themes + Ollama model catalog helpers (extracted to
// ``main-frontend/themes-catalog.js``). The factory owns all theme
// state mutations; the orchestrator only sees the returned public
// helpers and the ``themesCatalogState`` object's identity is
// preserved so any direct readers (none today, but possible in the
// future) observe the same array/Set references the helpers
// mutate.
// ============================================================================

const themesHelpers = createThemesCatalogHelpers({
  state: themesCatalogState,
  getCurrentSettings,
  DEFAULT_SETTINGS,
});
const {
...
const {
  ...
  resolveSubnetCalculatorPanel: () => globalThis.subnetCalculatorPanel,
});
...
function setSettingsSubtab(tabName = SETTINGS_SUBTAB_GENERAL) {
  ...
  if (themesPanel) {
    ...
    if (!themesPanel.hidden) {
      ...
      // ``isThemesCatalogReady`` is the factory-owned predicate that
      // checks the closure-private ``themesCatalogLoading`` /
      // ``themesCatalogEntries`` state — the orchestrator never
      // reads those directly.
      if (isThemesCatalogReady()) {
...
const {
  ...
  startupReleaseCheckPromise: () => startupReleaseCheckPromise,
  setStartupReleaseCheckPromise: (promise) => {
    startupReleaseCheckPromise = promise;
    window.__PACKETSNITCH_STARTUP_RELEASE_CHECK_PROMISE__ = promise;
  },
});
...
// Expose ``subnetCalculatorPanel`` on ``globalThis`` so the
// ``createSettingsDiagnosticsHelpers`` factory can resolve it via
// the ``resolveSubnetCalculatorPanel`` thunk passed at the call
// site. The factory runs at module top level (well before this
// declaration), so it cannot read the const directly.
globalThis.subnetCalculatorPanel = subnetCalculatorPanel;
