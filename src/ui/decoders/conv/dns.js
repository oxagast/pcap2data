...
// compression pointers (RFC 1035 §4.1.4): the high two bits of a length byte
// are `11`, the remaining 14 bits are an offset back into the message. When
// the pointer is followed the parser keeps walking the *target* labels and
// does not re-enter the compression state, so a single loop is sufficient.
// Returns { name, endIndex, ok }. `endIndex` is the index immediately after
// the consumed name in the original message (NOT in the dereferenced
// message). `ok` is false when the parse went out of bounds.
function decodeDnsName(bytes, startIndex, messageEnd) {
    ...
    while (cursor < messageEnd) {
        ...
        if ((length & 0xc0) === 0xc0) {
            // Compression pointer (14-bit offset). The pointer consumes two
            // bytes in the original message regardless of the jump.
            ...
            jumping = true;
            jumps += 1;
            if (jumps > maxJumps) return { name: "", endIndex: startIndex, ok: false };
            continue;
        }
        if ((length & 0xc0) !== 0) {
