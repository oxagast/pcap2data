...
// ============================================================================
const { bookmarkList } = require('../state');
...
// Settings diagnostic helpers are extracted to
// ``src/ui/main-frontend/settings-diagnostics.js`` (a
// ``createSettingsDiagnosticsHelpers`` factory). The orchestrator
// re-exports the helpers via ``const { ... } = createSettingsDiagnosticsHelpers(...)``.
// The test mirror is read-only (see
// ``tests/extraction_panel.test.js``); when something eventually
// drives it via ``require()`` these stubs satisfy the call sites
// without pulling the full factory + DOM deps into a Node-only
// sandbox.
function refreshOllamaStartupAvailability() { return false; }
function refreshMetricsDiagnostics() { return null; }
function refreshBackendDiagnostics() { return null; }
function refreshVirusTotalDiagnostics() { return null; }
function refreshHashesComDiagnostics() { return null; }
function invalidateVirusTotalDiagnosticsCache() { }
function invalidateHashesComDiagnosticsCache() { }
function recordHashesComLookupOutcome() { }
function syncLlmDiagnosticsIndicators() { }
function syncMetricsDiagnosticsIndicators() { }
function syncBackendDiagnosticsIndicators() { }
function getBackendVirusTotalApiKey() { return ""; }
function getBackendHashesComApiKey() { return ""; }
function getLlmDiagnosticElement() { return null; }
function renderLlmDiagnosticIndicator() { }
function getMetricsDiagnosticElement() { return null; }
function renderMetricsDiagnosticIndicator() { }
function getBackendDiagnosticElement() { return null; }
function renderBackendDiagnosticIndicator() { }
function syncVirusTotalDiagnosticsIndicators() { }
function syncHashesComDiagnosticsIndicators() { }
function formatRelativeTimestamp() { return ""; }
...
// Plugin management helpers are extracted to
// ``src/ui/main-frontend/plugin-management.js`` (a
// ``createPluginManagementHelpers`` factory). The orchestrator
// re-exports the helpers via ``const { ... } = createPluginManagementHelpers(...)``.
// The test mirror is read-only (see
// ``tests/extraction_panel.test.js``); when something eventually
// drives it via ``require()`` these stubs satisfy the call sites
// without pulling the full factory + DOM deps into a Node-only
// sandbox.
function getPluginManagerListElement() { return null; }
function getPluginCapabilityPanelMetaElement() { return null; }
function getPluginCapabilityPanelListElement() { return null; }
function getPluginErrorPanelElement() { return null; }
function normalizePluginCapabilityList() { return []; }
function getPluginCapabilityCatalogMap() { return new Map(); }
function createPluginCapabilityListItem() { return null; }
function renderSelectedPluginCapabilities() { }
function requestPluginInstallCapabilityDialog() { return Promise.resolve(false); }
function resolvePluginInstallCapabilityDialog() { }
function renderPluginErrorPanel() { }
function recordPluginError() { }
function clearPluginErrors() { }
function setPluginManagerMessage() { }
function resolvePluginEntryPath() { return ""; }
function reportPluginRuntimeFailure() { return Promise.resolve(); }
function loadInstalledPluginEntry() { return Promise.resolve(); }
function loadEnabledInstalledPlugins() { return Promise.resolve(); }
function createPluginRowElement() { return null; }
function renderPluginRegistryView() { }
function refreshPluginRegistryView() { return Promise.resolve(); }
function installPluginFromSettingsAction() { return Promise.resolve(); }
...
function buildSettingsChangeSummaries(previousSettings, nextSettings) {
  ...
  const previousApiKeys = previousSettings?.apiKeys || {};
  const nextApiKeys = nextSettings?.apiKeys || {};
  ...
  pushChange(
    ...
    previousApiKeys.virusTotalApiKey,
    nextApiKeys.virusTotalApiKey,
    { redacted: true },
  );
  pushChange(
  ...
  pushChange(
    "ollamaApiKey",
    previousApiKeys.ollamaApiKey,
    nextApiKeys.ollamaApiKey,
    { redacted: true },
  );
  pushChange(
    "activeByDefault",
...
function getCurrentSummaryReportMarkdown() {
  ...
  if (!notesSection) return summary;
  if (!summary) return notesSection;
  return `${summary}\n\n---\n\n${notesSection}`;
}
...
// tab; the underlying render call is a no-op on a hidden tab and will
// simply re-render the next time the analyst switches to it.
function refreshSummaryForNotes() {
  if (typeof renderCombinedAnalysisSummary === "function") {
    renderCombinedAnalysisSummary();
  } else if (typeof renderSummaryMarkdownPreview === "function") {
...
function getSummaryMarkdownForExport() {
  return normalizeSummaryMarkdownHeadings(getCurrentSummaryReportMarkdown());
}
...
document.getElementById("help-btn").addEventListener("click", async function () {
  ...
  // open the help page in the user's default system browser
  const helpUrl = "https://packetsnitch.com/docu/";
  writeLogEntry("Opening help page in default browser: " + helpUrl);
  if (window.browserapi && typeof window.browserapi.openExternalUrl === "function") {
    try {
      await window.browserapi.openExternalUrl(helpUrl);
      return;
    } catch (err) {
      console.error("openExternalUrl failed for help page, falling back to window.open:", err);
    }
  }
  // Fallback: open in a new window if the openExternalUrl bridge is unavailable
