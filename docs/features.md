...
- **Wi-Fi Keys (Keystore)**: per-session key list pulled from the keystore. Key types supported are `wifi-wep` (hex WEP key, 5/13/16 bytes for WEP-40 / WEP-104 / WEP-128), `wifi-wpa-psk` (WPA2 passphrase), and `wifi-pmk` (pre-computed PMK). Key entries can be added, edited, and removed directly from this panel.
...
- **WPA2 / AES-CCMP payload decrypt**: when a WPA2 4-way handshake is present in the capture, the backend derives the per-session PTK (PBKDF2-HMAC-SHA1 PMK → PRF-384 PTK per IEEE 802.11i §8.5.1) and uses the TK portion to decrypt CCMP data frames. Pre-computed PMK entries (`wifi-pmk`) and a 5-/13-/16-byte WEP key in the same keystore both reach the same `decryptWifiPayload` plumbing. Decrypted hex and ASCII preview plus a **Send to Conv** button.
- **WEP (Wired Equivalent Privacy) payload decrypt**: 802.11 frames whose protected bit is set on a `Dot11WEP` layer (or whose FC field is protected when the WEP layer is not parsed) are RC4-decrypted with the matching `wifi-wep` keystore entry. The decoder uses the proper WEP body layout (3-byte IV + 1-byte KeyID + ciphertext + 4-byte ICV) and runs the standard CRC-32 ICV check leniently (real WEP captures in the wild often have a corrupt or zeroed ICV), then gates the "ok" verdict on a structural sanity check — the plaintext must start with the 802.2 LLC / SNAP header (`DSAP=0xAA SSAP=0xAA Control=0x03`) and contain an IANA-assigned EtherType, or fail with a raw Ethernet-II header. The wrong key therefore never produces a false positive. The decryptor pulls ARC4 from `cryptography.hazmat.decrepit.ciphers.algorithms` (with a fallback to the legacy `cryptography.hazmat.primitives.ciphers.algorithms` path) so the path keeps working on both `cryptography >= 43` and older installs. A bundled `samples/pcaps/wep-A4-81-53-B4-CF.pcap` (BSSID `c0:4a:00:80:76:e4`, WEP-40 key `A4:81:53:B4:CF`) is shipped with the release for a one-click smoke test.
- **Link-layer protocol is never confused for the app protocol** — when a decrypted 802.11 frame is spliced back into the packet loop in [src/backend/snitch.py](src/backend/snitch.py), only the inner transport / application-layer protocol names flow into `packet.decoded_protocols`. Link-layer identity is conveyed via `link.proto = "IEEE 802.11"` instead. The List panel's **App Protocol** column additionally ignores link-layer protocol names (`WIFI`, `IEEE 802.11`, `ETHERNET`, `LINUX COOKED`, `FRAME`, …) when picking an application-layer label, so a decrypted TCP frame correctly surfaces `HTTP` / `SSH` / `DNS` rather than `WIFI`.
